...
export default function EmergencyCard({ data, lang }: Props) {
  ...
  const contacts =
    data.household.contacts?.filter((c) => c.name && c.phone) || [];
  const meetingPoint = loc?.shelters?.[0];
  const meetingDist = meetingPoint?.distance
    ? `${Math.round(meetingPoint.distance)}m`
    : "";
  ...
  const memberInfos = members
    ...
    .map((m) => {
      ...
      if (m.medications)
        medParts.push(zh ? `藥:${m.medications}` : `Meds:${m.medications}`);
      if (m.allergies)
        medParts.push(zh ? `過敏:${m.allergies}` : `Allergy:${m.allergies}`);
      ...
      <Page size={{ width: CARD_W, height: CARD_H }} style={s.page}>
        ...
        <View style={s.body}>
          ...
          {memberInfos.slice(0, 4).map((m, i) => (
            <View key={i} style={s.row}>
              ...
              <View style={{ flex: 1 }}>
                ...
                {m.medical ? (
                  <Text style={s.detailCol}>{m.medical}</Text>
                ) : null}
              </View>
            </View>
          ))}
          ...
          <View style={s.meetingBox}>
            <Text style={s.meetingLabel}>{zh ? "災後集合點" : "MEET AT"}</Text>
            <View style={{ flex: 1 }}>
            ...
            <View>
              {[1, 2].map((i) => (
                <View key={i} style={s.contactRow}>
                  ...
                  <Text style={[s.contactPhone, { color: "#d1d5db" }]}>
                    _______________
                  </Text>
                </View>
              ))}
