...
export default function InteractiveMap({
  ...
  const handleMapClick = useCallback(
    async (lat: number, lng: number, map: L.Map) => {
      ...
      clickMarkerRef.current = L.marker([lat, lng], { icon: CLICK_ICON }).addTo(
        map,
      );
      ...
      res.shelters.slice(0, 3).forEach((s) => {
        ...
        L.marker([s.lat, s.lng], { icon: icon("#3b82f6") })
          .bindPopup(
            `<b>${s.name}</b><br/>${s.address || ""}<br/>${s.distance ? Math.round(s.distance) + "m" : ""}`,
          )
          .addTo(group);
        ...
        <div className="absolute bottom-0 left-0 right-0 bg-white rounded-t-2xl shadow-2xl z-[1000] max-h-[55vh] overflow-y-auto border-t border-border">
          ...
          <div className="px-4 py-3 space-y-2.5">
            {result.shelters[0] && (
              <div className="flex items-start gap-2">
                ...
                <span className="text-xs font-medium text-primary shrink-0">
                  {fmt(result.shelters[0].distance)}
                </span>
              </div>
            )}
            ...
            {result.airRaid[0] && (
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[10px] text-text-faint">
                    {zh ? "防空避難" : "Air Raid Shelter"}
                  </p>
                  <p className="text-sm font-semibold text-text truncate">
                    {result.airRaid[0].name}
                  </p>
                  <p className="text-[11px] text-text-muted truncate">
                    {result.airRaid[0].address}
                  </p>
                </div>
                <span className="text-xs font-medium text-warning shrink-0">
                  {fmt(result.airRaid[0].distance)}
                </span>
              </div>
            )}
            ...
            {result.aed[0] && (
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm font-semibold text-text truncate">
                    {(result.aed[0] as AedLocation).name}
                  </p>
                </div>
                <span className="text-xs font-medium text-accent shrink-0">
                  {fmt(result.aed[0].distance)}
                </span>
              </div>
            )}
            ...
            {result.fireStation?.[0] && (
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm font-semibold text-text truncate">
                    {result.fireStation[0].name}
                  </p>
                  <p className="text-[11px] text-text-muted">
                    {result.fireStation[0].phone}
                  </p>
                </div>
                <span className="text-xs font-medium text-red-500 shrink-0">
                  {fmt(result.fireStation[0].distance)}
                </span>
              </div>
            )}
            ...
            {result.policeStation?.[0] && (
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm font-semibold text-text truncate">
                    {result.policeStation[0].name}
                  </p>
                  <p className="text-[11px] text-text-muted">
                    {result.policeStation[0].phone}
                  </p>
                </div>
                <span className="text-xs font-medium text-blue-600 shrink-0">
                  {fmt(result.policeStation[0].distance)}
                </span>
              </div>
            )}
          </div>
          ...
          <div className="px-4 pb-4 pt-1 space-y-2">
            <button
              ...
              className="w-full bg-primary text-white py-3 rounded-lg font-semibold text-sm hover:bg-primary-dark transition-all active:scale-[0.98]"
            >
              {zh
                ? "用這個位置製作完整手冊"
                : "Create handbook for this location"}
            </button>
            <button
              onClick={() => {
                setResult(null);
