...
import {
  CITY_DISASTER_LINKS,
  GENERAL_DISASTER_LINKS,
} from "@/lib/city-resources";

const ShelterMap = dynamic(() => import("@/components/ShelterMap"), {
  ssr: false,
  loading: () => (
    <div className="h-[300px] rounded-xl bg-surface animate-pulse" />
  ),
});
const MapCapture = dynamic(() => import("@/components/MapCapture"), {
  ssr: false,
  loading: () => null,
});
...
export default function HandbookPage() {
    ...
    try {
      ...
      if (raw) {
        try {
          ...
          console.error(
            "[handbook] sessionStorage handbookData is corrupted — clearing",
          );
          sessionStorage.removeItem("handbookData");
        }
      }
      const imgs = sessionStorage.getItem("mapImages");
      if (imgs) {
        try {
          setMapImages(JSON.parse(imgs));
        } catch {
  ...
  const recalcShelterDistance = async (
    locIdx: number,
    shelterIdx: number,
    type: "shelters" | "airRaid" | "medical",
  ) => {
    if (!data) return;
    ...
    if (geo) {
      ...
      const a =
        Math.sin(dLat / 2) ** 2 +
        Math.cos((loc.geo.lat * Math.PI) / 180) *
  ...
  const relocateLocation = async (locIdx: number) => {
    ...
    try {
      ...
      const loc = data.locations[locIdx];
      const geo = await geocode(relocateAddr.trim(), {
        city: loc.city,
      ...
      <div className="max-w-2xl mx-auto px-4 py-8 space-y-4">
        ...
        {data.locations[0]?.geo && (
          <div className="bg-white rounded-xl shadow-sm border border-border p-6">
            ...
            <ErrorBoundary label="MapCapture">
              <MapCapture
                locations={data.locations}
                onAllCaptured={setMapImages}
              />
            </ErrorBoundary>
          </div>
        )}
          ...
          {data.locations.map((loc, locIdx) => (
            ...
            >
              <div className="flex items-center gap-2 mb-1">
                ...
                <button
                  onClick={() => {
                    setRelocating(relocating === locIdx ? null : locIdx);
                    setRelocateAddr(loc.address);
                  }}
              ...
              {loc.shelters.length > 0 && (
                <div>
                  ...
                  {loc.shelters.map((sh, si) => (
                    <div
                      key={si}
                      className="flex items-start gap-2 py-1.5 border-b border-border/50 last:border-0 group"
                    >
                      <span className="text-text-faint text-xs font-bold mt-1 w-4 flex-shrink-0">
                        {si + 1}
                      </span>
                      <div className="flex-1 min-w-0">
                        {editing === `s-${locIdx}-${si}-name` ? (
                          <input
                            ...
                            value={sh.name}
                            onChange={(e) =>
                              updateShelter(locIdx, si, "name", e.target.value)
                            }
                          ...
                          <button
                            onClick={() => setEditing(`s-${locIdx}-${si}-name`)}
              ...
              {(loc.airRaid ?? []).length > 0 && (
                <div>
                  ...
                  {(loc.airRaid ?? []).slice(0, 3).map((sh, si) => (
                    ...
                    >
                      <span className="text-purple-400 text-xs font-bold mt-1 w-4 flex-shrink-0">
                        {si + 1}
                      </span>
                      <div className="flex-1 min-w-0">
                        {editing === `a-${locIdx}-${si}-name` ? (
                          <input
                            ...
                            value={sh.name}
                            onChange={(e) =>
                              updateAirRaid(locIdx, si, "name", e.target.value)
                            }
                            onBlur={() => setEditing(null)}
                            onKeyDown={(e) =>
                              e.key === "Enter" && setEditing(null)
                          ...
                          <button
                            onClick={() => setEditing(`a-${locIdx}-${si}-name`)}
                            className="text-sm text-text hover:text-primary text-left w-full truncate"
                          >
                            {sh.name}
                          </button>
                ...
                <div>
                  ...
                  {loc.medical.slice(0, 3).map((m, mi) => (
                    <div
                      ...
                      className="flex items-start gap-2 py-1.5 border-b border-border/50 last:border-0"
                    >
                      <span className="text-success/70 text-xs font-bold mt-1 w-4 flex-shrink-0">
                        {mi + 1}
                      </span>
                      <div className="flex-1 min-w-0">
                        {editing === `m-${locIdx}-${mi}-name` ? (
                          <input
                            ...
                            value={m.name}
                            onChange={(e) =>
                              updateMedical(locIdx, mi, "name", e.target.value)
                            }
                            onBlur={() => setEditing(null)}
                            onKeyDown={(e) =>
                              e.key === "Enter" && setEditing(null)
                          ...
                          <button
                            onClick={() => setEditing(`m-${locIdx}-${mi}-name`)}
                            className="text-sm text-text hover:text-primary text-left w-full truncate"
                          >
                            {m.name}
                          </button>
