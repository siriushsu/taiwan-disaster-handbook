...
- **Primary:** `#0D9488` — teal, inspired by Taiwan gov.tw. Reads as both trustworthy (blue) and natural (green). Used for header, buttons, active states, links. (Tailwind `teal-600`.)
- **Primary Dark:** `#0F766E` — hover states. (Tailwind `teal-700`.)
- **Primary Light:** `#F0FDFA` — focus rings, selected backgrounds, subtle highlights. (Tailwind `teal-50`.)
...
| 2026-04-24 | Primary teal aligned to `#0D9488` (teal-600) | Originally `#0D7377`. Shipped web drifted to Tailwind `teal-600` (`#0D9488`) while PDF stayed on `#0D7377`. Unified on the web value so site + PDF brand match; slightly lighter/more vibrant reads well on white. |
