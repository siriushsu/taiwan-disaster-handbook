...
async function fetchWithTimeout(url: string, timeoutMs = 30000): Promise<Response> {
  ...
  try {
    const res = await fetch(url, {
      signal: ctrl.signal,
      headers: { 'User-Agent': 'TaiwanDisasterHandbook/1.0 (data update script)' },
    })
    clearTimeout(timer)
    return res
  } catch (e) {
...
async function fetchCSV(url: string): Promise<string | null> {
  try {
    ...
    console.log(`    ✗ Failed: ${(e as Error).message} — ${url.slice(0, 80)}...`)
    return null
  }
}
...
async function searchDataGov(keyword: string): Promise<DataGovDataset[]> {
  try {
    ...
    return (data?.datasets || data?.result?.results || []).map((d: Record<string, unknown>) => ({
      datasetId: d.id || d.datasetId,
      title: d.title,
      resources: ((d.resources || []) as Record<string, unknown>[]).map(r => ({
...
async function updateShelters(dryRun: boolean) {
  ...
  const existing: Shelter[] = JSON.parse(fs.readFileSync(filePath, 'utf-8'))
  const existingByAddr = new Map(existing.map(s => [s.address, s]))
  const existingByName = new Map(existing.map(s => [`${s.district}_${s.name}`, s]))
  ...
  for (const source of SHELTER_SOURCES) {
    ...
    for (const row of rows) {
      ...
      const capacity = parseInt(findCol(row, SHELTER_COL_MAP.capacity)) || undefined
      ...
      if (existingByAddr.has(key)) {
        ...
        let changed = false
        if (capacity && capacity !== s.capacity) { s.capacity = capacity; changed = true }
        if (isValidTaiwanCoord(lat, lng) && !isValidTaiwanCoord(s.lat, s.lng)) {
          s.lat = lat; s.lng = lng; changed = true
        }
        if (changed) updated++
        ...
        const newShelter: Shelter = {
          ...
          lat, lng, capacity,
          disasterTypes: findCol(row, SHELTER_COL_MAP.disasterTypes) || undefined,
  ...
  console.log(`  ✓ Results: ${added} added, ${updated} updated, ${existing.length} total`)
...
const AIR_RAID_SOURCES = [
  { name: '臺北市', url: 'https://data.taipei/api/dataset/70a6216e-4855-4a76-8fb4-5c3e3ef771de/resource/3bd658e7-96c6-401e-9bdd-4cb0a61f86e4/download' },
  { name: '新北市', url: 'https://data.ntpc.gov.tw/api/datasets/3a9d87f0-1f10-4be4-8866-e7e1de4e9407/csv/file' },
  { name: '桃園市', url: 'https://opendata.tycg.gov.tw/api/dataset/12eb630b-b480-4b30-8b8e-5dda0bd785ed/resource/fb856832-3321-43dc-a1fd-18d0c0bbf1ff/download' },
  { name: '新竹市', url: 'https://odws.hccg.gov.tw/001/Upload/25/OpenData/9261/1a83861a-c2c2-4c5a-b08f-7d67db6ddf7c.csv' },
  { name: '臺中市', url: 'https://newdatacenter.taichung.gov.tw/api/v2/datasets/662dbb8c-e3cb-4833-a46f-e6f6ad4fd0ab/resource/download/rid/662dbb8c-e3cb-4833-a46f-e6f6ad4fd0ab' },
  { name: '彰化縣', url: 'https://www.chpb.gov.tw/opendata/8sGgzAHD40Kr1kYg7GSSAw/csv' },
  { name: '南投縣', url: 'https://data.nantou.gov.tw/dataset/fdd78983-fab0-44f7-9e32-0d89f1088277/resource/13cf6edc-a5e6-41e7-a71d-f75e23ad5dbe/download/20260305.csv' },
  { name: '雲林縣', url: 'https://www.ylhpb.gov.tw/df_ufiles/a/045-%E9%9B%B2%E6%9E%97%E7%B8%A3%E8%AD%A6%E5%AF%9F%E5%B1%80%E9%98%B2%E7%A9%BA%E7%96%8F%E6%95%A3%E9%81%BF%E9%9B%A3%E8%A8%AD%E6%96%BD%E4%B8%80%E8%A6%BD%E8%A1%A8.csv' },
  { name: '嘉義市', url: 'https://data.chiayi.gov.tw/opendata/api/getResource?oid=9add82b7-fe2b-40fa-8ad2-c05f5d4fc5f1' },
  { name: '嘉義縣', url: 'https://ws-tm.cyhg.gov.tw/Download.ashx?u=LzAwMS91cGxvYWQvNjgvcmVsZmlsZS84MjgxLzI5MjM3L2Y1ZDdmNDBmLTY3Y2ItNDFmNi1hMTRiLWRlOWY5ZThhMjk2NC5jc3Y%3d&n=5ZiJ576p57ij6Ziy56m655aP5pWj6YG%2f6Zuj6Kit5pa9LmNzdg%3d%3d' },
  { name: '臺南市', url: 'https://data.tainan.gov.tw/File/ResourceCsvDownload/e57347d7-d5fb-4ee3-9e98-c69da60f5fa5' },
  { name: '高雄市', url: 'https://data.kcg.gov.tw/File/directDownload/e5ee3906-be9c-440d-ab10-c352dff5e92b' },
  { name: '屏東縣', url: 'https://www-ws.pthg.gov.tw/001/upload/ebook/89b22d69-a6f6-4da5-b7de-a399a02c0530/resource/4c7a7a7d-9a5b-41c1-b8eb-3bebc3d7a4c7.csv' },
  { name: '宜蘭縣', url: 'https://opendataap2.e-land.gov.tw/api/v1/rest/datastore/c91e84872e88d1d5bb61e8a6d756d4c8.csv' },
  { name: '花蓮縣', url: 'https://ws.hl.gov.tw/Download.ashx?u=LzAwMS91cGxvYWQvNDIwL3JlbGZpbGUvMC80Mzc3MS9hYWVlYjc3Yi1mOWI0LTRiYTQtOTA2Ny1mMjYzZDQ1ODk3N2UuY3N2&n=6Iqx6JOu57ij6Ziy56m655aP5pWj6YG%2f6Zuj6Kit5pa9LmNzdg%3d%3d' },
  { name: '澎湖縣', url: 'https://opendataap2.penghu.gov.tw/api/v1/rest/datastore/f133972b077d368150506b88504099e6.csv' },
  { name: '金門縣', url: 'https://ws.kinmen.gov.tw/001/Upload/461/refile/13420/28908/af16a4f8-f29e-41f4-8bed-2f4a8e4ec0b1.csv' },
  { name: '保安警察第二總隊', url: 'https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/3C42D952-2AD3-4EA7-ABAE-2A75CCD0422A/resource/34181884-6D86-466A-AD14-BE6A0F9272F2/download' },
  { name: '苗栗縣', url: 'https://webws.miaoli.gov.tw/Download.ashx?u=LzAwMS91cGxvYWQvMjgvcmVsZmlsZS8xMjE2NC84NjkwMC8xZjAyYTRjOS04YjQwLTQ2MjMtOGY3My1jZjlhNWZlNTdlNTQuY3N2&n=6IuX5qCX57ij6K2m5a%2bf5bGA6Ziy56m655aP5pWj6YG%2f6Zuj6Kit5pa95L2N572u5riF5YaKLmNzdg%3d%3d' },
]
...
const AIR_RAID_COL_MAP: Record<string, string[]> = {
  ...
  capacity: ['容納人數', '可容納人數', '收容人數', '容量', 'capacity', '設施可容納人數'],
}
...
async function updateAirRaid(dryRun: boolean) {
  ...
  const existing: AirRaid[] = JSON.parse(fs.readFileSync(filePath, 'utf-8'))
  ...
  console.log(`  ✓ Results: ${added} added, ${updated} updated, ${failed} sources failed, ${existing.length} total`)
